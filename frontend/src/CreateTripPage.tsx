...
const CreateTripPage = () => {
  ...
  const coverError = useMemo(() => {
    if (coverFile) return ''
    if (!coverUrl.trim()) return 'Cover image file or URL is required'
    return ''
  }, [coverFile, coverUrl])

  const isFormValid = !titleError && !destinationError && !travelTypeError && !budgetError && !dateError && !coverError
      ...
      <section className="mx-auto max-w-4xl px-4 py-10">
        <div className="mb-8 rounded-3xl bg-white p-8 shadow-sm shadow-slate-200">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-3xl font-semibold text-slate-900">Create Trip</h1>
              <p className="mt-2 text-slate-600">Add a new trip and continue to the itinerary builder after saving.</p>
            </div>
            <a
              href="/trips"
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <label className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Add notes about the itinerary, trip highlights, and travel goals."
              />
            </label>
            ...
            >
              {submitting ? 'Saving trip...' : 'Create trip'}
            </button>
          </form>
        </div>
      </section>
